...
import { Search, Menu, GraduationCap, ChevronRight, X, Clock, Target, Brain, ArrowRight, Loader2 } from "lucide-react";
      ...
      <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md h-20' : 'bg-white h-24'}`}>
        <nav className="container-harvard h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            {isLoading ? (
              <Button className="bg-[#a51c30] hover:bg-[#821626] text-white rounded-none uppercase tracking-[0.25em] text-[12px] font-black px-10 h-14 transition-all flex items-center gap-3" disabled>
                <Loader2 className="w-5 h-5 animate-spin" />
                Cargando...
              </Button>
            ) : (
              <a href="/api/login" data-testid="btn-login-google">
                <Button className="bg-[#a51c30] hover:bg-[#821626] text-white rounded-none uppercase tracking-[0.25em] text-[12px] font-black px-10 h-14 transition-all flex items-center gap-3">
